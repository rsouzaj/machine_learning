---
title: "Atividade avaliativa 1 - Aula de introdução ao Aprendizado de Máquina"
...
Relação dos pacotes do R utilizados no trabalho
...
Tivemos que manipular o banco de dados no excel para modificar os delimitadores. 
Em português utilizamos a vírgula como delimitador de casa decimal e ponto para milhares.
...
**Diagnostico de Diabetes mellitus (DM)**
...
A variáveis preditoras foram selecionadas com base no artigo de Nascimento et. al.[@nascimento2003diabetes] A partir do [dicionário de variáveis](dicionario_de_variaveis_ex_ames_pns_2013.xlsx) do banco de dados.
      ...
      # regiao -> Região do país -> pode haver alguma relação com o tipo de 
      alimentação
            # 1 Norte
            # 2 Nordeste
            # 3 Sudeste
...
seus preenchimentos idênticos. Como a raça estava com preenchimento binário (1,2),
 assumimos que houve erro no preencimento dessa variável.
...
```
    ...
    a retirada do cálculo da média para imputação dos NAs. Mas vamos manter para 
    avaliação dos modelos preditivos e retirar somente se melhorarem a predição.
...
### Análise do IMC vs circunferência abdominal por diagnóstico de diabetes:
      ...
      Como esperado, se verificarmos a presença ou não de diabetes, a relação entre
      IMC e circunferência abdominal permanece.
...
## Tratamento do desbalanceamento na varável desfecho
...
Padronização das variáeis numéricas no banco de teste
...
Matriz de confusão dos dados de teste.
    ...
    Parece um bom modelo para identificar pessaos com alta probabilidade de ter diabetes.
